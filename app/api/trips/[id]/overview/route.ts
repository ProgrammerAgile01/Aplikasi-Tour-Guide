...
function buildSessionWindow(dateText: string, timeText: string) {
  ...
  const end = new Date(start.getTime() + 1 * 60 * 60 * 1000); // misal durasi 2 jam
...
//     // sesi yg belum benar-benar lewat
//     const futureOrOngoing = scheduleWindows
//       .filter((x) => x.start && x.end && x.end.getTime() >= now.getTime())
//       .sort((a, b) => a.start!.getTime() - b.start!.getTime());

//     // berapa yang sudah dihadiri
//     const completedCount = attendedSet
//       ? schedules.filter((s) => attendedSet!.has(s.id)).length
//       : 0;

//     const allCompletedByAttendance =
//       schedules.length > 0 && completedCount >= schedules.length;

//     const tripCompletedByStatus =
//       String(trip.status).toLowerCase() === "completed";

//     const noNextAgenda =
//       futureOrOngoing.length === 0 ||
//       allCompletedByAttendance ||
//       tripCompletedByStatus;
...
//       next = undefined;
//     } else if (attendedSet) {
//       // pilih sesi pertama yg BELUM dihadiri & belum lewat
//       const candidate = futureOrOngoing.find(
//         (x) => !attendedSet!.has(x.raw.id)
//       );
//       // ❗ tidak ada fallback ke sesi yang sudah dihadiri
//       next = candidate?.raw;
//     } else {
//       // user non-peserta (admin, dsb) → sesi terdekat yg belum lewat
//       next = futureOrOngoing[0]?.raw;
//     }
...
//       title: trip.name,
//       subtitle: trip.description ?? undefined,
//       nextAgenda: nextAgendaPayload, // bisa undefined kalau memang sudah selesai
//       todaysSummary: {
//         sessions: schedules.length,
//         completed: completedCount,
//         participants: Array.isArray(trip.participants)
