...
export async function GET(
  ...
  try {
    ...
    const payload: any = await getSessionFromRequest(req);
    if (!payload) {
      return NextResponse.json(
        { ok: false, message: "Not authenticated" },
        { status: 401 }
    ...
    // ===== 2. semua foto gallery peserta ini (APPROVED saja kalau mau) =====
    const galleries = await prisma.gallery.findMany({
      where: { tripId, participantId, deletedAt: null },
      orderBy: { createdAt: "asc" },
    });

    // sessionId -> 1 foto pertama
    const galleryBySessionId = new Map<string, string>();
    for (const g of galleries) {
      if (!galleryBySessionId.has(g.sessionId)) {
        galleryBySessionId.set(g.sessionId, g.imageUrl);
    ...
    const moments = attendances.map((a) => {
      ...
      const realImage = galleryBySessionId.get(a.sessionId) || null;
      const hasPhoto = !!realImage;
      ...
      return {
        ...
        time: s.timeText,
        checkedInAt: formatCheckedAt(a.checkedAt),
        // image yang dikirim ke FE: kalau ada foto pakai foto asli, kalau tidak pakai default
        image: realImage || DEFAULT_IMAGE,
        hasPhoto,
        caption: s.title,
        coordinates,
      };
