...
export default function FeedbackPage() {
  ...
  // sudah pernah submit? maka form akan readonly
  const hasSubmitted = !!myFeedback;
  ...
  const handleSubmit = async () => {
    if (hasSubmitted) return; // guard tambahan di FE
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-6 space-y-6">
      {/* Header */}
      <div className="text-center space-y-1">
        <h1 className="text-2xl font-bold text-foreground">Umpan Balik (Ulasan)</h1>
        <p className="text-sm text-muted-foreground">
          Kami menghargai masukan Anda untuk meningkatkan layanan kami
        </p>
      </div>
                ...
                return (
                  <button
                    ...
                    className={`transition-transform ${
                      !hasSubmitted ? "hover:scale-110" : "cursor-default"
                    }`}
                  >
                    <Star
                      size={40}
      ...
      <Card className="p-6 border border-border space-y-3">
        ...
        <textarea
          value={notes}
          onChange={(e) => !hasSubmitted && setNotes(e.target.value)}
          placeholder="Ceritakan pengalaman Anda lebih detail..."
          className="w-full px-3 py-2 rounded-lg border border-input bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 disabled:bg-muted disabled:text-muted-foreground"
          rows={5}
          disabled={hasSubmitted}
        />
      </Card>
      ...
      {!hasSubmitted ? (
        ...
        <Button
          disabled
          variant="outline"
          className="w-full py-3 text-base font-semibold cursor-default"
        >
