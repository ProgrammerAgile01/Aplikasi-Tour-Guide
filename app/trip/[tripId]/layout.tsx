...
import { Home, Calendar, Battery as Gallery, Star, LogOut, Map } from "lucide-react"
...
export default function TripLayout({
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 pb-20 overflow-y-auto bg-background">{children}</main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-card border-t border-border flex gap-0 shadow-lg z-50">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={24} />
              <span className="text-xs mt-1 font-medium text-center">{item.name}</span>
            </Link>
          )
        })}
      </nav>
