...
//               Anda telah menyelesaikan perjalanan luar biasa mengunjungi{" "}
//               {tripSummary.totalLocations} lokasi eksotis, mengabadikan{" "}
//               {tripSummary.totalPhotos} momen berharga, dan meraih{" "}
//               {tripSummary.badgesEarned} achievement badges.
//             </p>
//             <div className="pt-4 text-sm text-muted-foreground">
//               <p className="font-semibold">
//                 Terima kasih telah mempercayai perjalanan Anda bersama kami
//               </p>
...
export default function StoryPage() {
  ...
  useEffect(() => {
    ...
    const loadStory = async () => {
      try {
        ...
        const res = await fetch(`/api/trips/${tripId}/story`, {
          cache: "no-store",
        });

        const contentType = res.headers.get("content-type") || "";
        let json: StoryApiResponse;

        if (contentType.includes("application/json")) {
          json = (await res.json()) as StoryApiResponse;
        } else {
          const text = await res.text();
          console.warn("Non-JSON response:", text.slice(0, 200));
          throw new Error("Server mengembalikan respon HTML, bukan JSON");
        }

        if (!res.ok || !json.ok || !json.data) {
          throw new Error(json.message || "Gagal memuat story");
        }

        setTripSummary(json.data.tripSummary);
        setMoments(json.data.moments);
      } catch (err: any) {
        console.error("loadStory error:", err);
        setError(err?.message || "Gagal memuat story");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Story perjalanan Anda sedang diproses...",
      });

      const res = await fetch(`/api/trips/${tripId}/story-pdf`, {
        cache: "no-store",
      });

      if (!res.ok) {
        const text = await res.text();
        throw new Error(`Gagal generate PDF: ${text}`);
      }
      ...
      a.href = url;
      a.download = `trip-story-${tripId}.pdf`;
      ...
      toast({
        ...
        description: "File PDF story Anda sedang diunduh.",
      });
    } catch (err: any) {
      console.error("download pdf error:", err);
      toast({
        title: "Gagal mengunduh PDF",
        description: err?.message || "Terjadi kesalahan saat membuat PDF.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator
        .share({
          title: tripSummary?.title ?? "Trip Story",
          text: tripSummary
            ? `Cerita perjalanan saya: ${tripSummary.title}`
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
        <p className="text-sm text-slate-600">Memuat story perjalanan...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 px-4">
        <Card className="max-w-md w-full p-6 text-center">
          <h2 className="text-lg font-semibold mb-2">
            Story tidak dapat dimuat
          </h2>
          <p className="text-sm text-muted-foreground mb-4">{error}</p>
          <p className="text-xs text-muted-foreground">
            Pastikan Anda sudah melakukan presensi pada trip ini.
          </p>
        </Card>
      </div>
    );
  ...
  if (!tripSummary) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 px-4">
        <Card className="max-w-md w-full p-6 text-center">
          <h2 className="text-lg font-semibold mb-2">Belum ada story</h2>
          <p className="text-sm text-muted-foreground">
            Story perjalanan akan muncul setelah Anda melakukan presensi di
            beberapa lokasi pada trip ini.
          </p>
        </Card>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto space-y-8" ref={contentRef}>
        ...
        {moments.length === 0 ? (
          <Card className="p-6 text-center">
            <p className="text-sm text-muted-foreground">
              Belum ada lokasi yang terekam untuk story perjalanan ini.
            </p>
          </Card>
        ) : (
          <div className="space-y-6">
        ...
        <Card className="p-8 text-center bg-gradient-to-br from-white to-blue-50 border-2 border-primary/20">
          <div className="max-w-xl mx-auto space-y-4">
            ...
            <div className="pt-4 text-sm text-muted-foreground">
              ...
              <p className="mt-2 text-xs">Powered by Agile</p>
            </div>
          </div>
        </Card>
      </div>
